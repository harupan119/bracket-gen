...
var BracketGen = (() => {
  ...
  var COL_UNIT_PX = 7.5;
  function textPx(text, sizePt) {
    const em = sizePt * (4 / 3);
    let w = 0;
    for (const ch of String(text != null ? text : "")) w += /[\x20-\x7E]/.test(ch) ? em * 0.55 : em;
    return w;
  }
  function colUnits(px) {
    return Math.ceil(px / COL_UNIT_PX);
  }
  var CELL_PAD_PX = 10;
  var BOX_COL_UNITS = 20;
  function fullWidthFit(sizePt, units = BOX_COL_UNITS) {
...
${teams} \u30C1\u30FC\u30E0\u306A\u3089 format: single \u307E\u305F\u306F double \u3092\u9078\u3093\u3067\u304F\u3060\u3055\u3044\u3002`
  ...
  function buildDoubleElimination({ teams, bracketReset = true }) {
    ...
    for (let r = 1; r < wbLosers.length; r++) {
      ...
      const drops = pool.map((_, i) => {
        var _a2;
        return (_a2 = wbLosers[r][i]) != null ? _a2 : BYE;
      });
      const major = [];
    ...
    const lbChampion = (_c = pool[0]) != null ? _c : BYE;
...
\u7DCF\u5F53\u305F\u308A\u306E\u610F\u5473\u304C\u8584\u3044\u306E\u3067\u3001\u7D44\u6570\u3092\u6E1B\u3089\u3059\u304B\u30C1\u30FC\u30E0\u6570\u3092\u5897\u3084\u3057\u3066\u304F\u3060\u3055\u3044\u3002`
  ...
  function knownPlayers(match) {
    const out = [];
    for (const ref of [match.left, match.right]) {
      if ((ref == null ? void 0 : ref.type) === "team") out.push(`T${ref.index}`);
      else if ((ref == null ? void 0 : ref.type) === "groupRank") out.push(`G${ref.group}-${ref.rank}`);
    }
    return out;
  }
  ...
  var claim = (match, taken) => knownPlayers(match).forEach((p) => taken.add(p));
  ...
  function layoutMobileSheet(tournament) {
    ...
    const cardPx = Math.max(
      ...tournament.matches.map(
        (m) => textPx(`${refPlaceholder(m.left)} vs ${refPlaceholder(m.right)}`, THEME.sizes.body)
      )
    );
    g.setColumnWidth(1, 8).setColumnWidth(2, Math.max(22, colUnits(cardPx + CELL_PAD_PX))).setColumnWidth(3, 12).setColumnWidth(4, 14);
  ...
  function layoutProgressSheet(tournament) {
    ...
    g.set(
      ...
      1,
      `\u203B \u5168\u89D2${fullWidthFit(THEME.sizes.body)}\u6587\u5B57\u3092\u8D85\u3048\u308B\u3068\u3001\u30D6\u30E9\u30B1\u30C3\u30C8\u306E\u67A0\u306E\u4E2D\u3067\u6298\u308A\u8FD4\u3057\u307E\u3059`,
      { role: "note" }
    );
  ...
  var COLUMNS = [
    ...
    { col: 2, width: BOX_COL_UNITS },
    // B: 左チーム / ブラケット第1列
    { col: 3, width: 4 },
    // C: vs / 連結線（実物と同じ30px。ここを塗って経路の帯にする）
    { col: 4, width: BOX_COL_UNITS },
    // D: 右チーム / ブラケット第2列
    { col: 5, width: 4 },
    // E: 連結線
  ...
  function lastBracketCol(tournament) {
    ...
    if ((_b = (_a = tournament.loserTree) == null ? void 0 : _a.levels) == null ? void 0 : _b.length) cols.push(loserBracketCol(tournament));
  ...
  function layoutBracketSheet(tournament) {
    ...
    for (const group of groups) {
      ...
      g.set(base, 2, "\u9032\u51FA\u30C1\u30FC\u30E0", { role: "tableHeader" }).set(base, 3, "", { role: "tableHeader" }).set(base, 4, `${shortRound(group.semis[0].roundName)} ${semiLabels} \u306E\u52DD\u8005`, { role: "tableHeader" }).set(base, 5, "", { role: "tableHeader" }).set(base, 6, `${shortRound(group.final.roundName)} ${group.final.label}`, { role: "tableHeader" });
  ...
  function nextMatchOf(tournament, ref, parentRef) {
    ...
    if (ref && ref.type === "loser") {
      return (_d = (_c = tournament.matches.find((m) => m.id === ref.match)) == null ? void 0 : _c.loserTo) != null ? _d : null;
    }
    const same = (a, b) => {
    ...
    if (ref && (ref.type === "team" || ref.type === "groupRank")) {
      ...
      return (_e = m == null ? void 0 : m.id) != null ? _e : null;
    }
    return null;
  }
  function subtitle(t) {
    ...
    levels.forEach((level, j) => {
      level.forEach((ref, i) => {
        ...
        if (j > 0 && ref.type !== "winner") {
          const from = [i * 2, i * 2 + 1].map((k) => shownAt.get(`${j - 1},${k}`)).find(Boolean);
          shownAt.set(`${j},${i}`, from);
          if (from && next && up) {
            g.path(slotRow, slotRow, c, from, next);
            g.path(slotRow, up.row, c + 1, from, next);
          }
          return;
        }
  ...
  function loserTreeNodes(levels) {
    ...
    for (let j = 1; j < levels.length; j++) {
      ...
      for (let i = 0; i < lv.refs.length; i++) {
        ...
        const dropRef = minor ? null : (_b = (_a = lv.drops) == null ? void 0 : _a[i]) != null ? _b : null;
        const b = minor ? nodes[2 * i + 1] : dropRef ? { kind: "leaf", ref: dropRef } : null;
        next.push(a && b ? { kind: "match", ref: lv.refs[i], a, b, drop: !minor } : (_c = a != null ? a : b) != null ? _c : null);
      }
      nodes = next;
    }
    return (_d = nodes[0]) != null ? _d : null;
  }
  function loserDepth(node) {
    return node && node.kind === "match" ? Math.max(loserDepth(node.a), loserDepth(node.b)) + 1 : 0;
  }
  function loserBracketCol(tournament) {
    var _a;
    const levels = (_a = tournament.loserTree) == null ? void 0 : _a.levels;
    if (!(levels == null ? void 0 : levels.length)) return 0;
  ...
  function renderLoserTree(g, tournament, startRow, inBracket) {
    ...
    const merges = [];
    let cursor = base + 1;
    const place = (node, col2) => {
      if (node.kind === "leaf") {
        const slot = { ref: node.ref, row: cursor, col: col2 };
        cursor += LOSER_ROW_STEP;
        entries.push(slot);
        return slot;
      }
      const a = place(node.a, col2 - 2);
      let b;
      if (node.drop) {
        b = { ref: node.b.ref, row: a.row + LOSER_ROW_STEP, col: col2 - 2 };
        entries.push(b);
        cursor = Math.max(cursor, b.row + LOSER_ROW_STEP);
      } else {
        b = place(node.b, col2 - 2);
      }
      const out = { ref: node.ref, row: Math.round((a.row + b.row) / 2), col: col2 };
      merges.push({ a, b, out });
      return out;
    };
  ...
  var absolute = (ref) => String(ref).replace(/\$/g, "").replace(/^([A-Z]+)(\d+)$/, "$$$1$$$2");
